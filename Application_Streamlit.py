# streamlit_app.py — version TURQUOISE premium (UX/UI soigné, images 100% gratuites)
# ---------------------------------------------------------------------------------
# Lancer :
...
# ============================
# STYLES — thème vert turquoise (clair, premium)
# ============================
CUSTOM_CSS = """
<style>
:root{
  --turq:#00BFA6;         /* turquoise principal */
  --turq2:#00D2C6;        /* turquoise clair */
  --emerald:#10B981;      /* vert émeraude */
  --coral:#FF6B6B;        /* corail (alerte/faux) */
  --ink:#0F172A;          /* bleu très sombre pour texte fort */
  --muted:#475569;        /* gris bleuté */
  --bg1:#ECFEFF;          /* bleu très clair */
  --bg2:#F0FFFC;          /* vert très clair */
  --card:#FFFFFF;
  --bd:#E6F6F3;           /* bordure douce */
}

/* fond dégradé subtil turquoise */
.stApp{background: radial-gradient(1200px 700px at 20% 0%, var(--bg1) 0%, #fff 60%),
                         linear-gradient(120deg, var(--bg2), #ffffff 60%);} 
.block-container{padding-top:1rem;}

/* cartes glass */
.glass{background:var(--card);border:1px solid var(--bd);border-radius:18px;box-shadow:0 10px 28px rgba(0,191,166,.08);padding:1.1rem 1.2rem}
...
.stButton>button{border-radius:12px;font-weight:700;padding:.7rem 1.1rem;border:1px solid var(--bd);
                 background:linear-gradient(90deg,var(--turq),var(--turq2)); color:#fff}
...
# ============================
# IMAGES (100% gratuites, Pexels/Unsplash CDN stables)
# ============================
HERO_IMG = "https://images.pexels.com/photos/6266518/pexels-photo-6266518.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=1600"  # UV check money (Pexels)
GRID_IMG_1 = "https://images.pexels.com/photos/4386370/pexels-photo-4386370.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=1200"  # graphs over money (Pexels)
GRID_IMG_2 = "https://images.pexels.com/photos/730547/pexels-photo-730547.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=1200"    # money + charts (Pexels)
GRID_IMG_3 = "https://images.pexels.com/photos/15206825/pexels-photo-15206825.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=1200"  # close-up dollar bills (Pexels)            # macro banknotes (Unsplash)
...
with st.sidebar:
    ...
    api_base = st.text_input("URL de l'API", value="http://127.0.0.1:5001")
    mode = st.radio("Mode d'envoi", ["Fichier CSV (multipart)", "JSON (records)"])
    endpoint = st.text_input("Endpoint", value="/predict_csv/")
    st.markdown("---")
...
if uploaded is not None:
    ...
    if st.button("🚀 Lancer les prédictions", use_container_width=True):
        try:
            ...
            # --- STATISTIQUES (bar chart horizontal au lieu du donut)
            with tabs[1]:
                total = len(out)
                    ...
                    st.markdown(
                        ...
                        <div class='glass metric-card'>
                          ...
                          <div class='metric-body'>
                            <div class='badge-pill' style='background:#E6FFFA;'>Total</div>
                            <h3>{total} lignes</h3>
                          </div>
                        </div>
                        """, unsafe_allow_html=True)
                with c2:
                    st.markdown(
                        ...
                        <div class='glass metric-card'>
                          <div class='metric-emoji'>✅</div>
                          <div class='metric-body'>
                            <div class='badge-pill' style='background:#E8FFF6;color:#0f5132;'> {label_true} </div>
                            <h3>{n_true}</h3>
                          </div>
                        </div>
                        """, unsafe_allow_html=True)
                with c3:
                    st.markdown(
                        ...
                        <div class='glass metric-card'>
                          ...
                          <div class='metric-body'>
                            <div class='badge-pill' style='background:#FFF0F0;color:#842029;'> {label_false} </div>
                            <h3>{n_false}</h3>
                          </div>
                        </div>
                        """, unsafe_allow_html=True)

                # Bar chart horizontal (lisible & moderne)
                if "Classe" in out.columns:
                    bar_df = out["Classe"].value_counts().rename_axis("Classe").reset_index(name="Nombre")
                    # assurer ordre Vrai/Faux si souhaité
                    domain = [label_true, label_false] if label_true in bar_df["Classe"].values else list(bar_df["Classe"].values)
                    range_colors = ["#00BFA6", "#FF6B6B"] if len(domain)>=2 else ["#00BFA6"]
                    bar = (
                        alt.Chart(bar_df)
            ...
            with tabs[2]:
                if "Probabilité_classe_1" in out.columns:
                    ...
                    # Densité lissée (courbe) — look premium
                    dens = alt.Chart(out).transform_density(
                        "Probabilité_classe_1",
                        as_=["Probabilité_classe_1", "Densité"],
                        extent=[0,1],
                        steps=200
                    ).mark_area(opacity=0.35).encode(
                        x="Probabilité_classe_1:Q",
                        y="Densité:Q",
                        color=alt.value("#00D2C6")
                    ...
                    line = base.mark_line(point=alt.OverlayMarkDef(filled=True, size=45)).encode(
                        ...
                        color=alt.Color("Classe:N", scale=alt.Scale(domain=[label_true,label_false], range=["#00BFA6","#FF6B6B"]), title="Classe"),
                        tooltip=["index", "Probabilité_classe_1", "Classe"],
                    )
                    st.altair_chart(line, use_container_width=True)

                # Scatter features — diag vs length (utile pour patterns)
                if set(["diagonal","length"]).issubset(out.columns):
                    scatter = alt.Chart(out).mark_circle(size=80, opacity=0.75).encode(
                        ...
                        color=alt.Color("Classe:N", scale=alt.Scale(domain=[label_true,label_false], range=["#00BFA6","#FF6B6B"]), legend=alt.Legend(title="Classe")),
            ...
            with tabs[4]:
                st.markdown(
                    ...
                    **Notes**
                    ...
                    Vous vous êtes sûrement déjà demandé si un billet était vrai. 
                    Avec notre application, finie l'incertitude ! 
                    Nous avons développé un outil puissant, mais facile à utiliser, 
                    qui transforme votre téléphone en un détecteur de faux billets. 
                    Notre objectif est de vous permettre de vérifier vos billets rapidement, en toute confiance, où que vous soyez.
